...
# bin/fm-progress-lib.sh can read.
#
...
# release drops its token meter, or renames it into a shape the library does not
# match, the report does not fail loudly - it goes quiet for that harness and
# falls back to the hour-long completed-turn bound, which is exactly the gap the
# report was built to close. A stubbed pane cannot see that happen: it can only
...
#
# Cost: one short real turn per installed harness (a single word), sampled while
# it runs. That is the smallest turn that can produce a counter at all, and it
# is the price of not silently losing supervision coverage.
set -u
...
# an observation that it does render counters is what promotes it into this
...
for harness in claude codex opencode pi pi-signed grok kimi cursor muse; do
  ...
  # Sample across the turn: a counter that appears for only part of it still
  # proves the reading exists, and sampling is what the watcher does too.
  found=
  sample=
  for _ in $(seq 1 40); do
    sample=$("$REAL_TMUX" -L "$SOCKET" capture-pane -p -t "$target" 2>/dev/null || true)
    if reading=$(fm_progress_counters "$sample"); then
      found=$(printf '%s' "$reading" | tr '\n' ' ')
      break
    fi
    sleep 1
  done
  ...
  if [ -n "$found" ]; then
    ...
    note "$harness $version: renders progress counters [$found]"
    pass "progress counters: $harness $version renders a readable progress counter"
  else
    WITHOUT_COUNTERS="$WITHOUT_COUNTERS $harness"
    case "$EXPECTED_COUNTERS" in
      *" $harness "*)
        fail "PROGRESS-COUNTER DRIFT: $harness $version rendered no counter bin/fm-progress-lib.sh can read during a real turn, though this repo's recorded evidence says it used to. The busy-but-no-progress report is silently blind for this harness and falls back to the hour-long completed-turn bound. Last pane sampled:
$sample"
        ;;
      *)
        note "$harness $version: renders no readable progress counter; this harness keeps the completed-turn bound as its only backstop"
        pass "progress counters: $harness $version observed (no counter rendered, recorded as an admitted blind spot)"
        ;;
    esac
  fi
  CHECKED=$((CHECKED + 1))
...
[ -n "$WITH_COUNTERS" ] || note \
  "no installed harness rendered a readable counter: the busy-but-no-progress report can measure nothing on this machine"
note "renders counters:${WITH_COUNTERS:- none}"
note "renders none:${WITHOUT_COUNTERS:- none}"
if [ -n "$SKIPPED" ]; then
