...
Because that exclusion makes those rows invisible to main, `bin/fm-guard.sh`'s queued-wake warning counts only the rows the calling actor can itself present or retire (`fm_wake_actor_pending_count`, whose implementation `bin/fm-wake-lib.sh` owns alongside the grant row-list and owner-record reads that the drain and `bin/fm-wake-grant.sh` share): a row a live grant reserves is never counted as drainable for main, so main is never sent to a drain that can only print nothing; instead the guard prints a distinct advisory naming the live supervision branch as the holder and saying not to drain those rows from here, and a main drain repeats that hold in its own held notice. The branch actor's queued-wake output stays suppressed in every case.
