...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling, a merge-queue ejection, or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pull request the merge queue later ejects returns to `OPEN`, which the merge poll alone cannot tell from one still waiting its turn, so that poll also reports the ejection as its own actionable check output instead of leaving a blocked delivery to look like a queued one.
[`bin/fm-pr-enqueue.sh`](../bin/fm-pr-enqueue.sh)'s header owns the bounded automatic re-queue that answers such an ejection, the reasons that escalate to the captain instead, and the fact that a re-queue is not a merge and records no merge outcome.
