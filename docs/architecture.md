...
A pool worktree is only returned after teardown passes the slot-ownership proof: a contradictory task record naming the same copy - the canonical physical directory, whatever spelling each record used - refuses without touching either task, and no discard authority relaxes that.
The same exclusivity check runs before signalling processes or removing any ship or scout worktree - whatever its backend, not only a recognized pool slot - and on the forced-teardown descendant path as well.
