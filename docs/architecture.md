...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, busy panes whose rendered progress counters have not moved for `FM_BUSY_NO_PROGRESS_SECS`, and heartbeat backstop hits.
...
That bound can only ask how long a turn has run, which cannot separate a wedged worker from one on a single long legitimate call, so it has to be set generously and cannot see a worker frozen inside a turn.
A second, independent measure covers that case: `bin/fm-progress-lib.sh` reads the harness's own rendered progress counters - up and down token meters, spend, and context - which move only when the model actually consumes or produces, and deliberately excludes elapsed clocks, spinners, and percentages that do not name the context window, because a frozen harness keeps repainting all of those and the whole-pane hash changes with them.
Two readings of those counters spaced `FM_BUSY_NO_PROGRESS_SECS` apart separate a worker that is advancing from one that is not, so the fuse can be far shorter than the completed-turn bound without firing on ordinary long work.
It is a report and nothing else: its own wording, never the wedge wording, and no interrupt, signal, restart, or teardown.
It stands down for away mode, where the daemon owns triage, for a declared external wait or verified captain-held transfer whose standstill is already explained, for a task whose authoritative state is an actively running no-mistakes run-step (legitimately waiting on its upstream review or CI monitor with nothing moving in its own pane), for a task whose own worktree was written during the frozen window, and for any pane whose numbers have never been observed advancing.
That last exemption is what keeps displayed content from being read as a meter: matching a counter shape proves only that the text looks like one, and a transcript line inside the footer region matches exactly as a real meter does while telling nothing about whether this worker is advancing.
A pane's numbers must therefore be seen ADVANCING at least once - one of them rising, which is what a meter does within the first seconds of generation and what displayed content does not, since text scrolling through the footer region changes between polls but is as likely to fall as to rise - before the short fuse applies to it.
Until then, and permanently for a harness that renders no counters, the pane is recorded in the watcher's absorbed-wake log as an admitted blind spot rather than guessed at, and keeps `FM_BUSY_TURN_MAX_SECS` as its only backstop.
That evidence belongs to the agent that earned it and lasts exactly as long: every reading the measure keeps is bounded by the incarnation that took it (the recorded spawn generation and the semantic busy generation, both minted by a spawn or a relaunch and both untouched by the ordinary mid-life writers of a task's spawn record), so a respawn of the same id, or a relaunch onto another runtime, starts unarmed on a window whose markers it inherited rather than being measured against a meter it never rendered.
