...
It stands down for away mode, where the daemon owns triage, for a declared external wait or verified captain-held transfer whose standstill is already explained, for a task whose own worktree was written during the frozen window, and for any harness that renders no counters at all - that last case is recorded in the watcher's absorbed-wake log as an admitted blind spot rather than guessed at, and keeps `FM_BUSY_TURN_MAX_SECS` as its only backstop.
