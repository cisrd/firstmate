...
A queued signal annotation prints every status line still unread at that cursor, while the fleet-wide UNREAD STATUS section prints `note:` lines, reserved-key resolutions, and close attempts the reserved-key guard rejected once even on an empty-queue drain, because none of those lines stay in the OPEN DECISIONS fold as themselves; an accepted `captain-held` transfer is excluded there because the durable captain-held ledger already presents it, and `fm-classify-lib.sh`'s `status_line_is_unread_surface` owns that membership rule.
