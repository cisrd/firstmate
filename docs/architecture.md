...
It stands down for away mode, where the daemon owns triage, for a declared external wait or verified captain-held transfer whose standstill is already explained, for a task whose own worktree was written during the frozen window, and for any pane whose numbers have never been observed changing.
That last exemption is what keeps displayed content from being read as a meter: matching a counter shape proves only that the text looks like one, and a transcript line inside the footer region matches exactly as a real meter does while being static forever, which is also what a frozen counter looks like.
A pane's numbers must therefore be seen changing at least once - behaving like a meter rather than merely looking like one - before the short fuse applies to it, which static content can never do and a real meter does within the first seconds of generation.
...
That evidence belongs to the agent that earned it and lasts exactly as long: it is stamped with the incarnation (the task's spawn record and semantic busy generation) and retired as soon as either changes, so a respawn of the same id, or a relaunch onto another runtime, starts unarmed on a window whose markers it inherited rather than being measured against a meter it never rendered.
