...
A pool worktree is only returned after teardown passes the slot-ownership proof: a contradictory task record naming the same copy - by physical path, inode, or git worktree identity - refuses without touching either task, and no discard authority relaxes that.
The same exclusivity check runs before signalling processes or removing any ship or scout worktree, not only a recognized pool slot.
