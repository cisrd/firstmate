...
A registry entry with `integration-branch=<branch>` makes that declared branch the sync target even when `origin/HEAD` names another default branch.
For that declaration, fleet sync fetches and compares only `origin/<branch>`, then fast-forwards the checked-out local `<branch>` when safe.
A legacy entry without the declaration retains remote-default resolution and its existing sync behavior.
