...
A home that opted into outbound ntfy notifications also projects that classified span onto the notifier's fixed event catalog, which reads the leading verbs and records a durable publication intent locally; it is an observer of the verdict and never changes triage, wake eligibility, or the classified position ([`ntfy-notifications.md`](ntfy-notifications.md), and `bin/fm-ntfy-lib.sh`'s header for the mechanics).
