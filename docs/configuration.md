...
FM_SECONDMATE_WAKE_STALL_SECS=180  # minimum interval with no advance of the oldest actionable foreign wake-queue sequence before an endpoint-recorded local secondmate produces one durable parent wake-loop-stall notification for that no-progress episode; a mate that is provably inside an active turn (busy verdict within FM_BUSY_TURN_MAX_SECS of a completed turn) never escalates whatever this interval says, declared external-wait pause rows are excluded, and zero or invalid values use 180
