...
FM_BUSY_NO_PROGRESS_SECS=1500      # seconds a BUSY pane may go with every rendered progress counter unchanged before the watcher reports it as busy-but-not-advancing; measured from the harness's own token, spend, and context meters (bin/fm-progress-lib.sh), never from elapsed time or the pane hash, so a worker that keeps generating resets it continuously however long its turn runs; report-only, with its own wording, and never an interrupt, signal or restart; a zero, negative, or non-numeric value falls back to this default rather than reporting every poll or silently disabling the measure; it stands down entirely for away mode, a declared external wait, a verified captain-held transfer, a task whose own worktree was written during the frozen window, and any pane whose numbers have never been observed changing - that last case covers both a harness that renders no counters and one whose only counter-shaped text is static displayed content, is recorded in the watcher's absorbed-wake log, and keeps FM_BUSY_TURN_MAX_SECS as its only backstop
