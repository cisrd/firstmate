...
| `fm-ntfy.sh`             | The optional outbound ntfy pager: `status`, the standing delivery `check`, a transport `test`, and `arm`/`disarm` for the watcher cadence ([ntfy-notifications.md](ntfy-notifications.md)) |
| [`fm-ntfy-lib.sh`](../bin/fm-ntfy-lib.sh) | Own what the pager may publish, its minimal projection, and its durable at-least-once delivery with firstmate-side de-duplication |
