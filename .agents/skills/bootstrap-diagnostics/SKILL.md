...
- `WORKTREE_COLLISION: live <path> claimed by <id> (<detail>), ...` - two or more claimants' own agent processes are still hazardous (alive, or a state that cannot be read - an unverifiable process is always a hazard, including over a path that no longer exists), so two task records genuinely still contend for one worktree; this is the hazard the check exists for.
  The kind is decided by process state alone; the shared path's own state never promotes a collision to `live`, and never withholds a line either - every path claimed by two or more local records prints exactly one line, whatever its state.
...
- `WORKTREE_COLLISION: stale <path> claimed by <id> (<detail>), ...` - at most one claimant's process is still hazardous; the rest are finished tasks whose records were never cleaned up, most often a pooled worktree recycled by a later spawn.
  Each `(<detail>)` describes only that claimant's own process - `process alive`, `process state unknown (backend=<name> not verifiable)`, or `process gone` - and never says whose work is at the path.
  A `process state unknown` detail names the backend that could not answer, which separates an execution engine firstmate cannot classify at all from one that is merely unreachable right now; resolve that before deciding the record is finished, because an unverifiable claimant is counted as a hazard, not as a leftover.
...
- A `stale` line ending `- shared path still has unlanded work, do not discard` adds one fact about the SHARED PATH, not about any named claimant: the worktree is dirty or its HEAD is not reachable from the default branch, most often the surviving live claimant's own work in progress.
  Unlanded work never makes the line say `live`, so treat this caveat as the same do-not-discard warning under a `stale` label - it is how the check keeps that hazard from going silent.
  Reconcile the finished claimant's record only; never clean, reset, or remove the path itself to clear the line.
- A line ending `- shared worktree no longer exists at that path` adds the other path-level fact, on either kind: the recorded worktree is not there any more, so every named claimant is pointing at something that was already torn down or moved.
  Reconcile each named record's own bookkeeping per the bullets above; a vanished path is never a reason to assume the claimants are finished, and a claimant still reported `process alive` there is a genuine anomaly worth the captain's attention.
- Every WORKTREE_COLLISION line covers LOCAL task records only: a record carrying `remote_host=` is skipped, because its `worktree=` names a path on another machine that is unique only when host-qualified, and a local git or backend probe would report a systematically wrong verdict about it.
  So this check is not evidence that remote secondmates share no home; those homes are dedicated rather than pooled, and their double-registration is guarded by the secondmate registry's own `ssh:<host>:<home>` key instead.
