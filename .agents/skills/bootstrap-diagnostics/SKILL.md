...
- `WORKTREE_COLLISION: live <path> claimed by <id> (<detail>), ...` - two or more task records still contend for one worktree; this is the hazard the check exists for (`bin/fm-worktree-collision-lib.sh`'s header owns what each field asserts, including the local-records-only scope - read it there rather than inferring the contract from the line).
...
- `WORKTREE_COLLISION: stale <path> claimed by <id> (<detail>), ...` - at most one claimant is still a hazard; the rest are finished tasks whose records were never cleaned up, most often a pooled worktree recycled by a later spawn.
...
- A line ends with one path caveat whenever the shared path is not proven safe to reclaim, on either kind.
  `still has unlanded work, do not discard`, `not an inspectable git worktree, so whether work would be lost cannot be verified, do not discard`, and `whose working-tree state could not be read, so whether work would be lost cannot be verified, do not discard` all mean the same thing for your actions: reconcile records only, and never clean, reset, remove, or return the path itself to clear the line.
  The second is the same condition `bin/fm-teardown.sh` refuses on, so escalate rather than forcing it.
  The third means git resolved a worktree there but could not read its working-tree state at all - most often a damaged `.git/index` from an agent killed mid-git-operation - so repair or inspect that copy by hand before trusting any claim about what it holds.
