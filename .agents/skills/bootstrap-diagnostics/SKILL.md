...
- `WORKTREE_COLLISION: live <path> claimed by <id> (<detail>), ...` - two or more claimants' own agent processes are still hazardous (alive, or a state that cannot be read over a path that still exists), so two task records genuinely still contend for one worktree; this is the hazard the check exists for.
  The kind is decided by process state alone; the shared path's own state never promotes a collision to `live`.
...
- `WORKTREE_COLLISION: stale <path> claimed by <id> (<detail>), ...` - at most one claimant's process is still hazardous; the rest are finished tasks whose records were never cleaned up, most often a pooled worktree recycled by a later spawn.
  Each `(<detail>)` describes only that claimant's own process - `process alive`, `process state unknown`, or `process gone` - and never says whose work is at the path.
