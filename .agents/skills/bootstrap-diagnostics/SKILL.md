...
- A line ends with one path caveat whenever the shared path is not proven safe to reclaim, on either kind. `still has unlanded work, do not discard` and `not an inspectable git worktree, so whether work would be lost cannot be verified, do not discard` both mean the same thing for your actions: reconcile records only, and never clean, reset, remove, or return the path itself to clear the line. The second is the same condition `bin/fm-teardown.sh` refuses on, so escalate rather than forcing it.
