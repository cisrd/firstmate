...
- `WORKTREE_COLLISION: live <path> claimed by <id> (<detail>), ...` - two or more claimants' own agent processes are still hazardous (alive, or a state that cannot be read), so two task records genuinely still contend for one worktree; this is the hazard the check exists for.
