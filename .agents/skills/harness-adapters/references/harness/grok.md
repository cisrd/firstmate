...
| Fact | Value |
|---|---|
| Busy state | The last rendered-tail fallback, isolated to Grok pending a semantic source: ASCII mid-turn `Ctrl+c:cancel`, absent from idle bar `Shift+Tab:mode │ Ctrl+.:shortcuts`, never the locale-fragile braille spinner. Known stale on 1.0.13, whose active footer no longer contains `Ctrl+c:cancel` under either captured launch shape, so a genuinely busy turn reads as not-working on every backend until the signature is repaired; herdr masks it only while its native agent state reads `working`, because the short-circuit ahead of this fallback fires on a native busy verdict alone and a long foreground tool call reads native idle. See [`docs/verification/grok-interrupt.md`](../../../../../docs/verification/grok-interrupt.md) for the captures and the affected consumers. |
...
| Interrupt | Single `Ctrl+C`, retained as the established path. Escape is version-dependent: it only focused scrollback on 0.2.73, while on 1.0.13 one Escape also visibly cancels the turn under the production launch shape. Neither key reliably stops already-running tool work on that build: Escape stopped the tool child in two production-shape trials and left it running in a third, and no `Ctrl+C` control was captured with the tool call as the pane's displayed active item, so the keys are untied on that axis rather than matched; on every axis actually compared they behave the same, so nothing favours switching. See [`docs/verification/grok-interrupt.md`](../../../../../docs/verification/grok-interrupt.md). |
