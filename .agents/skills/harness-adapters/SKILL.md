...
| grok | `--model <model>` | `--reasoning-effort <low\|medium\|high\|xhigh>` | Verified 2026-08-25 on grok 1.0.5 (5115b46bc9). `--effort` is an alias, but firstmate's profile axis is reasoning effort. `xhigh` is accepted; `max` is still rejected with `use one of: xhigh, high, medium, low`, so firstmate omits `max`. Refresh by running `tests/fm-spawn-dispatch-profile.test.sh` on a machine with grok on PATH. |
...
## grok (VERIFIED 2026-06-29, grok 0.2.73; slash-submit re-verified 2026-07-03 on 0.2.82; reasoning-effort ceiling re-verified 2026-08-25 on 1.0.5; exit paths re-verified 2026-07-19 on grok 0.2.103)
