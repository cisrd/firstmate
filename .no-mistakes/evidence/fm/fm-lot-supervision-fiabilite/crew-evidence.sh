...
. "/home/awilliam/.no-mistakes/worktrees/a03e7f5d4084/01M20Y4BVJS67Y30T1QYD7T8AW/tests/lib.sh"
...
pass() {
  printf '\n=== %s ===\n' "$*"
  if [ -n "${out:-}" ]; then
    if [ -f "$out" ]; then cat "$out"; else printf '%s\n' "$out"; fi
  fi
  if [ -n "${armout:-}" ] && [ -f "$armout" ]; then cat "$armout"; fi
}

eval "$(declare -f run_crew_state | sed '1s/run_crew_state/original_run_crew_state/')"
run_crew_state() { local result rc; result=$(original_run_crew_state "$@"); rc=$?; printf '%s\n' "$result" >&2; printf '%s\n' "$result"; return "$rc"; }
test_no_run_live_pane_agent_gone_keeps_terminal_log
test_no_run_live_pane_agent_gone_keeps_open_status_states
test_no_run_live_pane_agent_gone_stale_working_is_unknown
test_agent_gone_outranks_a_busy_lifecycle_record
