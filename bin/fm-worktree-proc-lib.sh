...
# hands a recorded path straight to the scan clears no wall at all. Every caller
# that may SIGNAL into a root therefore resolves it through one of the validated
# entry points first, and there is no longer an exception: bin/fm-teardown.sh
# runs both recorded values through fm_wtproc_signalling_root in its own
# validate_recorded_reap_roots, before any refusable step of the cleanup, and
