...
# "resolved [key=<key>]: answered: <capped excerpt>". A reserved key such as
# pending-reply-* receives the namespace vocabulary generated by
# status_decision_close_note in bin/fm-classify-lib.sh, so the same public
# interface closes every key the fold can open without teaching fm-send an
# owner-specific grammar. If the fold would reject the generated note, or the
# structural key would be lost to the status-line cap, fm-send refuses before
# sending and names the cause rather than exiting 0 on a silent no-op. After a
# delivered close it also
...
fm_send_add_resolve_key() {  # <key>
  local k=$1
  if ! status_decision_key_valid "$k"; then
...
if [ -n "$RESOLVE_KEYS" ]; then
  ...
  # Refuse before send when the shared grammar cannot produce an effective
