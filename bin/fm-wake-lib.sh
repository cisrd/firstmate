...
fm_wake_secondmate_progress_marker_write() { # <task> <observed-at> <oldest-seq>
  local task=$1 observed_at=$2 oldest_seq=$3 marker tmp
  case "$task" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  case "$observed_at" in ''|*[!0-9]*) return 1 ;; esac
  case "$oldest_seq" in ''|*[!0-9]*) return 1 ;; esac
  marker="$STATE/.secondmate-wake-progress-$task"
  if [ -e "$marker" ] || [ -L "$marker" ]; then
    [ -f "$marker" ] && [ ! -L "$marker" ] || return 1
  fi
  tmp=$(mktemp "$STATE/.secondmate-wake-progress.XXXXXX") || return 1
  if ! printf '%s\t%s\n' "$observed_at" "$oldest_seq" > "$tmp" || ! chmod 0600 "$tmp" \
    || ! _fm_atomic_replace "$tmp" "$marker"; then
    rm -f -- "$tmp"
    return 1
  fi
