...
# Every list below is built with `LC_ALL=C sort`, so every comm that merges two
# of them must run in that same collation. Left to inherit the caller's, comm
# rejects its own C-sorted input with "input is not in sorted order" wherever
# the locale collates differently (macOS and most interactive shells), and the
# guard exits non-zero having reported nothing about lane coverage.
run_coverage_guard() {
